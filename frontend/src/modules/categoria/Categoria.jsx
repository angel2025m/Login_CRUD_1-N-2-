...
const Categoria = () => {
    ...
    const AbrirModalEditar =(categoria)=>{
        SetModoEdicion(true)
        SetCategoriaEditada(categoria.id)
        SetformData({
    ...
    return(
        <div className="p-4 md:p-0">
                ...
                <table className="table-auto w-full border-separate border-spacing-x-5">
                    <thead>
                        <tr>
                            ...
                            <th className="bg-gray-300 border-gray-400 rounded-xl font-mono text-sm md:text-base">descripción</th>
                    ...
                    <tbody>
                        {
                         categorias.map((categoria)=>(
                            <tr key={categoria.id}>
                                <td className="font-mono text-center py-2 text-sm md:text-base">{categoria.id}</td>
                                <td className="font-mono text-center text-sm md:text-base">{categoria.nombre}</td>
                                <td className="font-mono text-center text-sm md:text-base">{categoria.descripcion || 'Sin descripción'}</td>
                                <td className="flex justify-center gap-2 items-center py-2">
                                    ...
                                    <Trash onClick={()=>EliminarCategorias(categoria.id)} className="hover:text-red-700 cursor-pointer" size={20}/>
                                </td>
                            </tr>
                         ))   
                        }
            ...
            {
                Abrir &&(
                    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 md:p-0">
                        <div className="bg-amber-50 w-full md:w-96 rounded-2xl">
                            {
                                modoEdicion?(
